#!/usr/bin/env python3
"""
Serves the homework sheet UI and looks up word pictures under word-images/
...
def main() -> int:
    ...
    host = os.environ.get("HOMEWORK_SHEET_HOST", "127.0.0.1")
    preferred = int(os.environ.get("HOMEWORK_SHEET_PORT", "8765"))
    ...
    if httpd is None:
        print(
            ...
            "Quit other Homework Sheet Generator windows or run: lsof -i TCP:{preferred}",
            file=sys.stderr,
        )
        return 1

    url = f"http://{host}:{port}/"
    print("Homework sheet generator")
    ...
    def open_browser() -> None:
        if os.environ.get("HOMEWORK_SHEET_NO_OPEN") == "1":
            return
        try:
